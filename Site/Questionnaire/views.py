...
from Tools.views import re_start_server
...
class Test(APIView):
    ...
    def get(self,request):
        ...
        print(BASE_DIR)
        STATIC_ROOT = os.path.join('aaa/', 'bb/b/','ccc')
        print(STATIC_ROOT)
        return Response('xxx')
...
class BatchTeacherViewSet(APIView):
    ...
    def get(self,request,*args,**kwargs):
        return Response({'下载模板':'{}/static/批量注册老师.xlsx'.format(settings.ROOT_URL)})
...
#批量的为班级添加问卷--通过id参数
class BatchClassQuestionnaireOfIdViewSet(MyModelViewSet):
    queryset = Class.objects.get_queryset().order_by('id')
    serializer_class = ClassSerializer

    def create(self, request, *args, **kwargs):
        if request.data.get('append',None):return self.create_append(request, *args, **kwargs)
        args_mapping={'province_id':'gradeclass__grade__schoolgrade__school__cityschool__city_obj__province_obj__id',
                      'city_id':'gradeclass__grade__schoolgrade__school__cityschool__city_obj__id',
                      'school_id':'gradeclass__grade__schoolgrade__school__id',
        ...
        for i in request.data:
            if request.data[i]:
                filter_args[args_mapping[i]]=request.data[i]
        self.queryset=Class.objects.filter(**filter_args)
        serializer=self.serializer_class(self.queryset,many=True)
        return Response(serializer.data)

    def create_append(self,request, *args, **kwargs):
        class_ids=request.data.get('class_ids')
        questionnaire_id=request.data.get('wenjuan_id')
        class_ids=eval(class_ids)
        class_objs=Class.objects.filter(id__in=class_ids)
        questionnaire_obj =Questionnaire.objects.filter(pk=questionnaire_id).first()
        success_info=[]
        for class_obj in class_objs:
            info=self.create_classquestionnaire(class_obj,questionnaire_obj)
            success_info.append(info)
        return Response(success_info)

    def create_classquestionnaire(self,class_obj,questionnaire_obj):
        classquestionnaire_obj=ClassQuestionnaire.objects.filter(class_obj=class_obj,
                                                                 questionnaire_obj=questionnaire_obj).first()
        if classquestionnaire_obj:
            return {'banji':str(class_obj),'wenjuan':questionnaire_obj.questionnaire_name}
        else:
            classquestionnaire_obj =ClassQuestionnaire.objects.create(class_obj=class_obj,
...
class ClassViewSet(MyModelViewSet):
    serializer_class = ClassSerializer
    queryset = Class.objects.get_queryset().order_by('class_name')
    AutoFilter={}
    CustomFilter={'学校':'gradeclass__grade__schoolgrade__school__school_name__contains',
                  '年级':'gradeclass__grade__grade_name__contains',
...
class CityViewSet(MyModelViewSet):
    ...
    queryset = City.objects.get_queryset().order_by('city_name')
...
class GradeViewSet(MyModelViewSet):
    ...
    queryset = Grade.objects.get_queryset().order_by('grade_name')
...
class ProvinceViewSet(MyModelViewSet):
    ...
    queryset = Province.objects.get_queryset().order_by('province_name')
...
class SchoolViewSet(MyModelViewSet):
    ...
    queryset = School.objects.get_queryset().order_by('school_name')
...
class UsersRolesViewSet(MyModelViewSet):
    ...
    def find__handler(self,data):
        print('====',data)
        queryset=self.queryset
        from django.db.models import Q
        queryset =queryset.filter(Q(user__name__contains=data)|Q(user__mobile__contains=data))
...
#验证老师账户
class ValidateTeacherOfHrefViewSet(APIView):

    def get(self,request,*args,**kwargs):
        # password = id * 44 - 9    yes
        # password = id * 44 - 10   no
        id=int(kwargs.get('id'))
        password=int(kwargs.get('password'))

        status=self.get_status(id,password)
        if status==1:
            teacher_obj = Teacher.objects.filter(pk=id).first()
            teacher_obj.is_valied = True
            teacher_obj.save()
            send_info = '{},您已成功注册成为青葱道教师用户,您的用户名为：{},你可以点击以下网址，{},登陆青葱道校园评估系统.'.format(
                teacher_obj.user.name, teacher_obj.user.mobile,'{}/index'.format(settings.CODE_URL))
            send_status = self.send_sms(teacher_obj,send_info)
            info = '{}已经通过验证,{}'.format(teacher_obj.user.name, send_status)
            return Response(info)
        elif status==2:
            teacher_obj = Teacher.objects.filter(pk=id).first()
            send_info = '{},非常抱歉,您未通过青葱道的网站的注册审核,本功能仅对相关校园用户开放,' \
                        '如有疑问请联系021-54712208,谢谢！'.format(teacher_obj.user.name)
            send_status = self.send_sms(teacher_obj, send_info)
            info = '{}未能通过验证,{}.用户已被删除.'.format(teacher_obj.user.name, send_status)
            teacher_obj.delete()
            return Response(info)
        else:
            raise serializers.ValidationError('错误的密钥')


    def send_sms(self,teacher_obj,send_info):
        mobile = teacher_obj.user.mobile
        sms=SendSMS(mobile=mobile,data=send_info)
        sms=sms.send()
        if sms:
            send_info = '并成功发送了通知短信'
        else:
            send_info='但是短信发送失败了'
        return send_info

    def get_status(self,id,password):
        if (password+9)/44==id:
            return 1
        elif (password+10)/44==id:
            return 2
        else:
            return 3
...
class VailedTeacherViewSet(MyModelViewSet):
    ...
    queryset = Teacher.objects.get_queryset().order_by('id')
    # CustomFilter = {'名字':'user__name__contains','年龄':'user__age','性别':'user__sex',
    #                 '学校':'classteacher__class_name__gradeclass__grade__schoolgrade__school__school_name__contains',
    #                 '班级':'classteacher__class_name__class_name__contains',
    #                 '年级':'classteacher__class_name__gradeclass__grade__grade_name__contains',
    #                 '已验证':'is_valied'}
    AutoFilter = {'is_valied':False}
    def list(self, request, *args, **kwargs):
        queryset = self.filter_queryset(self.get_queryset())
        serializer = self.get_serializer(queryset, many=True)
        # return Response("<a href='http://www.baidu.com'>百度</a>")
        infos=[]
        for info in serializer.data:
            info=self.info_maker(info)
            infos.append(info)
        return HttpResponse(infos if infos else '暂时没有需要审核的信息')
    ...
    def info_maker(self,info):
        status='此用户已经通过审核' if info.get('is_valied') else '此用户暂时没有通过审核'
        info_0='<br>+-----------------------------------------------------<br>'
        info_1='|老师姓名：{}<br>'.format(info.get('teacher_name'))
        info_2='|手机号码：{}<br>'.format(info.get('mobile'))
        info_3='|学校：{}<br>'.format(info.get('school'))
        info_4='|年级：{}<br>'.format(info.get('grade'))
        info_5='|班级：{}<br>'.format(info.get('class_name'))
        info_51='|状态：{}<br>'.format(status)
        info_6='|{}<a href={}>点此审核通过</a>'.format('&nbsp;'*5,info.get('yes'),)
        info_7='{}<a href={}><font color="red">点此审核不通过并且删除这个用户</font></a><br>'.format('&nbsp;'*5,info.get('no'),)
        info_8='+-----------------------------------------------------<br>'
        info=info_0+info_1+info_2+info_3+info_4+info_5+info_51+info_6+info_7+info_8
        return info
