...
class CodesSerializer(serializers.ModelSerializer):
    ...
    def validate(self,data):
        ...
        if the_type==1:
            if user:
                raise serializers.ValidationError('手机号已经被,请更换手机号注册')
...
class QuestionnaireSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields=('id','questionnaire_url','questionnaire_name','questionnaire_type',
                'school_name_field','grade_name_field','class_name_field','student_name_field',
...
class QuestionnaireMediaSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        school_field,grade_field,class_field,student_field= \
        ...
        # 3.定位参与者
        student_objs=self.custom_get_student_obj(school_name, grade_name, class_name, student_name)
    ...
    def custom_get_school_grade_class_field(self,questionnaire_obj):
        ...
        return school_field,grade_field,class_field,student_field

    # 3.定位参与者
    def custom_get_student_obj(self,school_name,grade_name,class_name,student_name):
        #去掉两边空格
        options=[]
        for option in [school_name,grade_name,class_name,student_name]:
            option=option.strip()
            options.append(option)
        school_name, grade_name, class_name, student_name=options
...
class VailedTeacherSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=Teacher
        fields=('id','teacher_name','school','class_name','grade','is_valied','info',)
